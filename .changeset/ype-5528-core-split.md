...
Behavior note: input-validation failures now throw zod Mini's `$ZodError` (from `zod/mini`) instead of classic `ZodError`. Custom validation messages are unchanged, but `instanceof ZodError` checks against classic `zod` no longer match, and default (non-custom) messages differ. Callers that catch validation errors should match on `error instanceof $ZodError` from `zod/mini` (or `z.core.$ZodError`) instead.
