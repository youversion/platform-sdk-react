...
- **CI** — [`.github/workflows/commitlint.yml`](../.github/workflows/commitlint.yml) runs on PRs and anchors the lint range at the **live tip of `origin/main`** (`git fetch origin main` → `commitlint --from <origin/main sha> --to <head.sha>`), not the PR's recorded base SHA. The recorded base goes stale as `main` advances and would re-lint already-merged commits; anchoring at `origin/main` lints only commits new to the branch.
