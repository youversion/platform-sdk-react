...
describe('VerseActionPopover', () => {
  ...
  describe('Viewport width cap + scrollable swatch row', () => {
    ...
    it('engages the edge-fade mask on scroll once the row overflows', () => {
      ...
      // Fake a row clipped on both physical edges, then fire a real scroll
      // event. Only an attached handler will measure it and apply the mask.
...
it('renders the RTL logical-start fade on the physical right edge', () => {
  ...
  vi.spyOn(swatches[swatches.length - 1]!, 'getBoundingClientRect').mockReturnValue(rect(268, 300));
